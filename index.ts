...
`);
  ...
  // Helper function to handle web_search tool
  async function handleWebSearch(args: {
    ...
    try {
      const currentToken = getCurrentToken();
      if (!currentToken) {
        throw new Error("handleWebSearch: No token found");
      }
      apiKey = await getApiKey(currentToken.access_token);
    } catch (error) {
      console.error(error);
      return `ERROR: this tool is currently not available, notify user about this and do not call this tool again.`;
    }
    ...
    let type = JSON.parse(atob("WyJHT09HTEUiLCJCSU5HIl0"));
    if (args.type?.toUpperCase() == "SCHOLAR") {
      type = JSON.parse(atob("WyJTQ0hPTEFSIl0"));
    } else if (args.type?.toUpperCase() == "PDF") {
      type = JSON.parse(atob("WyJQREYiXQ"));
    }

    const body = JSON.parse(
      atob(
        ""
      )
    );
    body.query = args.query;
    body[atob("c2VhcmNoRW5naW5lTGlzdA")] = type;

    const response = await fetch(
      atob("aHR0cHM6Ly9hcGlzLmlmbG93LmNuL3YxL2NoYXQvcmV0cmlldmU"),
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  // Helper function to handle web_download tool
  async function handleWebDownload(args: { url: string }): Promise<string> {
    try {
      // Parse url
      const origin = new URL(args.url).origin;

      // Merge custom headers with default headers
      const mergedHeaders = { ...defaultHeaders, Origin: origin };
  ...
  // Helper function to handle web_reader tool
  async function handleWebReader(args: { url: string }): Promise<string> {
    try {
      // Parse url
      const origin = new URL(args.url).origin;
  ...
  // Function to handle MCP server requests
  async function handleMcp(req: Request): Promise<Response> {
    ...
    try {
              ...
              tools: [
                {
                  ...
                  inputSchema: {
                    ...
                    properties: {
                      ...
                      type: {
                        type: "string",
                        description: "The scope of the search request",
                        default: "WEB",
                        enum: ["WEB", "SCHOLAR", "PDF"],
                      },
                    },
                    required: ["query"],
                  },
                },
                {
                  name: "web_download",
                  description:
                    "Fetch and read the content of a url. Only support text content, for example, json or txt files.",
                  inputSchema: {
                    type: "object",
                    properties: {
                      url: {
                ...
                },
                {
                  name: "web_reader",
                  description:
                    "Read the content of a webpage. This is useful when you need to read the content of a webpage without getting all the source code for the webpage. It is recommended to use web_reader to read further about a search result.",
                  inputSchema: {
                    type: "object",
                    properties: {
                      url: {
      ...
      if (body.method === "tools/call") {
        ...
        switch (toolName) {
          case "web_search":
            result = await handleWebSearch(
              toolArgs as { query: string; type: string }
            );
            break;
          case "web_download":
            result = await handleWebDownload(toolArgs as { url: string });
            break;
          case "web_reader":
            result = await handleWebReader(toolArgs as { url: string });
            break;
          default:
            return new Response(
              JSON.stringify({
