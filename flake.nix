        ...
        in
        {
          ...
          options.services.waterfall = {
            ...
            databasePath = mkOption {
              type = types.str;
              default = "/var/lib/waterfall/keys.db";
              description = ''
          ...
          config = mkIf cfg.enable {
            ...
            systemd.tmpfiles.rules = [
              # Create the directory for the database with proper ownership and write permissions
              "d '${dirOf cfg.databasePath}' 0770 waterfall waterfall - -"
              # Create the database file if it doesn't exist with write permissions
              "f '${cfg.databasePath}' 0660 waterfall waterfall - -"
            ];
            ...
            systemd.services.waterfall = {
              ...
              serviceConfig = {
                ...
                # Ensure the service can write to the database directory
                ReadWritePaths = [ "/var/lib/waterfall" ];
              };
            };
        ...
        in
        {
          waterfall = pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              ...
              #!${pkgs.runtimeShell}
              # Don't cd to $out/lib/waterfall - it's read-only
              # The service will set WorkingDirectory to /var/lib/waterfall
              exec ${pkgs.bun}/bin/bun $out/lib/waterfall/index.js "\$@"
              EOF
